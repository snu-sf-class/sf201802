...
(* Parameter array_new : forall (n:nat) (v:nat), array. *)
(* Parameter array_len : forall (a:array), nat. *)
(* Parameter array_write : forall (a: array) (i v: nat), array.  *)
(* Parameter array_read : forall (a: array) (i: nat), nat. *)

(* Axiom array_new_len : forall n v, array_len (array_new n v) = n. *)
(* Axiom array_write_len : forall a i v *)
(*     (RANGE: i < array_len a),  *)
(*   array_len (array_write a i v) = array_len a. *)
(* Axiom array_read_write : forall a i v  *)
(*     (RANGE: i < array_len a),  *)
(*   array_read (array_write a i v) i = v. *)
(* Axiom array_overwrite : forall a i v w  *)
(*     (RANGE: i < array_len a),  *)
(*   array_write (array_write a i v) i w = array_write a i w. *)
(* Axiom array_write_comm : forall a i j v w  *)
(*     (RANGEi: i < array_len a)  *)
(*     (RANGEj: j < array_len a)  *)
(*     (NEQ: i <> j),  *)
(*   array_write (array_write a i v) j w = array_write (array_write a j w) i v. *)

(* Definition Marr (T: Type) : Type := array -> array * T. *)

(* Global Instance Monad_array : Monad Marr := *)
(* { ret := fun  _ x => fun a => (a, x) *)
(* ; bind := fun _ _ c f => fun a => let (a',v) := c a in f v a' *)
(* }. *)

(* Definition MAlen : Marr nat := fun a => (a, array_len a). *)
(* Definition MAread i : Marr nat := fun a => (a, array_read a i). *)
(* Definition MAwrite i v : Marr unit := fun a => (array_write a i v, tt). *)

(* Fixpoint readL {m} `{NArray m} i n : m (list nat) := *)
(*   match n with *)
(*   | 0 => ret [] *)
(*   | S n' => tl <- readL i n';; hd <- read (i+n');; ret (hd::tl) *)
(*   end. *)

(* Fixpoint writeL {m} `{NArray m} i (l: list nat) : m nat := *)
(*   match l with *)
(*   | [] => ret i *)
(*   | hd::tl => j <- writeL i tl ;; write j hd ;; ret (j+1) *)
(*   end. *)
...
(* Definition find_min_arr_body (i: nat) : Marr unit := *)
(*   e1 <- MAread i;; *)
(*   e2 <- MAread (i+1);; *)
(*   if e1 <? e2 then *)
(*     MAwrite i e2;; *)
...
(* Definition find_min_arr (l: list nat) : list nat := *)
(*   let n := length l in *)
(*   MArun n 0 ( *)
(*     MAwriteL 0 l;; *)
(*     MAfor 0 (n-1) (fun i => *)
...
Proof.
  intros. induction PERM; simpl; try bdestruct (x =? e); firstorder.
Qed.
...
Proof.
  intros. induction H; inv H0; eauto.
  inv H3. eauto.
Qed.
...
Proof.
  intros hd tl. revert hd. induction tl; intros.
  - inv RES. eauto.
  - simpl in *. destruct (find_min_rest a tl) eqn: EQ.
    hexploit IHtl; eauto. intros IH.
    bdestruct (hd <=? n); inv RES; eauto using Forall_impl with arith.
Qed.
...
Proof.
  ...
  - simpl in *. destruct (find_min_rest a tl) eqn: EQ.
    hexploit IHtl; eauto. intros IH.
    bdestruct (hd <=? n); inv RES; eauto using Permutation, Permutation_trans.
Qed.
...
Proof.
  assert (AUX: forall n l, Permutation l (bubble_sort_aux n l)); eauto.
  induction n; intros; eauto.
  destruct l; eauto.
  simpl. destruct (find_min_rest n0 l) eqn: EQ.
  eauto using Permutation, find_min_rest_permutation.
Qed.
...
Lemma find_min_rest_length : 
  forall hd tl min rest
    (RES: find_min_rest hd tl = (min,rest)),
  length rest = length tl.
Proof.
  intros. 
  apply find_min_rest_permutation in RES.
  apply Permutation_length in RES.
  simpl in *. nia.
Qed.
...
Proof.
  assert (GEN: forall n l (LE: length l <= n), sorted (bubble_sort l)); eauto.
  induction n; intros; destruct l; eauto; simpl in LE; [nia|].
  unfold bubble_sort. simpl. destruct (find_min_rest n0 l) eqn: EQ.
  hexploit (IHn l0); [apply find_min_rest_length in EQ; nia|]. unfold bubble_sort. intros SORTED.
  erewrite <-find_min_rest_length; eauto.
  destruct l0; eauto.
  simpl in *. destruct (find_min_rest n2 l0) eqn: EQ'.
  econstructor; eauto.
  apply find_min_rest_minimum in EQ.
  apply find_min_rest_permutation in EQ'.
  eapply Forall_perm in EQ'; eauto.
  inv EQ'. eauto.
Qed.
...
Definition sorting {m} `{NArray m} : m unit := 
  n <- len;;
  NAfor 0 (n-1) (fun i =>
    NAfor 0 (n-i-1) (fun j =>
      e1 <- read j;;
